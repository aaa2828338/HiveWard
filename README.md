...
<p align="center">
  把 OpenClaw、Claude Code、Codex，以及 Google CLI Beta、Cursor CLI Beta、OpenCode Beta、Hermes Beta 组织成一家可调度、可审批、可复盘的 Agent Company。
</p>
...
它不是一个新的聊天框，而是一人公司的 Agent Company 工作台：把 OpenClaw、Claude Code、Codex，以及 Google CLI Beta、Cursor CLI Beta、OpenCode Beta、Hermes Beta 这样的 harness 当成可组织的员工，让它们在公司、CEO、Leader、蓝图、审批和历史记录里协同起来。

## 为什么选择 HiveWard？

HiveWard 的定位绝不是对抗 OpenClaw、Claude Code、Codex，或 Google CLI Beta、Cursor CLI Beta、OpenCode Beta、Hermes Beta 这些新增 Beta CLI harness，也不是代替 Skill 或 n8n。而是站在它们之上一层：把单打独斗的强力 harness 打造成可调度、可组织、可审批、可复用、可审计的业务蓝图节点。

| 维度 | HiveWard | Multica | Paperclip | OpenClaw | Claude Code + Skill | n8n |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 自由构建多 Agent 组织架构 | ⭐⭐⭐ | ⭐ | ⭐ | ⭐ | ⭐ | ⭐⭐⭐ |
| 跨 OpenClaw / Claude Code / Codex / Beta CLI harness 调度 | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐ | ⭐ | ⭐ |
| 保留原生 harness 能力 | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ | ⭐ |
| 工作流中人工审批/治理/微调 | ⭐⭐⭐ | ⭐⭐ | ⭐ | ⭐⭐⭐ | ⭐ | ⭐ |
| Agent 自主判断与循环 | ⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐ | ⭐ | ⭐⭐ | ⭐ |
| 上下文压力控制 | ⭐⭐⭐ | ⭐⭐ | ⭐⭐ | ⭐ | ⭐ | ⭐ |
...
蓝图是实际干活的调度图。你在画布上安排 Agent、Manager、Slot、汇总、条件和审批节点。真正承接工作的节点都是 Harness Agent：可以选择 OpenClaw、Codex、Claude Code 作为运行来源，也可以试用 Google CLI Beta、Cursor CLI Beta、OpenCode Beta、Hermes Beta 这些 Beta CLI harness。它们会尽量继续使用各自原生的模型、工具、技能、会话、权限和工作目录能力；HiveWard 不把这些员工削成简化版聊天框，它做的是总控和编排。
...
所以实际流程是：先建公司，再让 CEO 组织 Leader；再为每条业务线创建蓝图；再把每个蓝图节点绑定到合适的 Harness Agent；最后由 HiveWard 负责调度、监控、审批和沉淀历史。OpenClaw、Claude Code、Codex 和新增 Beta CLI harness 负责把事情真正做完，HiveWard 负责让它们像一个团队一样被组织起来。
...
graph LR
  ...
  Leader[业务 Leader]
  Blueprint[业务蓝图]
  Harness[OpenClaw / Codex / Claude Code / Beta CLI Harness Agent]
  Run[运行监控]
  Approval[人工审批]
  History[历史复盘]
...
当前 beta 版本的 CLI chat harness 默认使用安全模式，不会自动授予完整的文件读写、命令执行、联网和实时网页搜索权限。
