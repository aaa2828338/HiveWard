...
当前 beta 版本的聊天 Harness 默认使用完全访问权限。Codex 和 Claude Code 聊天 Harness 可以读写当前工作区文件、执行命令、访问网络，并使用实时网页搜索。这让 HiveWard 更接近原生 CLI 的完整能力，但也意味着模型可能修改文件、运行脚本、读取本地上下文或向外部网络发起请求。

请只在你信任的本地仓库和运行环境中使用默认完全访问模式；不要在包含敏感凭据、生产密钥或不可恢复文件的目录中直接运行。
