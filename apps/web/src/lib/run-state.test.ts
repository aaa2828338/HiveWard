...
describe("run state sync", () => {
  ...
  it("derives reply conversation state from a revised approval request", () => {
    ...
    runView.approvalDecisions = [
      {
        ...
        action: "reply",
        actor: "user",
        comment: "Focus on tech industry news.",
    ...
    expect(approvals[0]).toMatchObject({
      approvalRequestId: "approval-plan",
      nodeRunId: "approval-plan",
      nodeLabel: "Top Manager",
      reviewOutput: "revised execution plan",
      replies: [
        {
          id: "decision-1:user",
          role: "user",
          body: "Focus on tech industry news.",
          createdAt: "2026-05-21T01:03:00.000Z"
        },
        {
          id: "approval-plan:revision-2",
          role: "assistant",
          body: "revised execution plan",
          createdAt: "2026-05-21T01:04:00.000Z",
          selected: true
  ...
  it("keeps a requirement approval reply visible while the manager revision is still running", () => {
    const runView = createRunView("running");
    runView.approvalRequests = [
      {
        id: "approval-plan",
    ...
    expect(approvals[0]).toMatchObject({
      approvalRequestId: "approval-plan",
      status: "replying",
      reviewOutput: "initial execution plan",
      canApprove: false,
      canReply: false,
      canReject: false,
      replies: [
