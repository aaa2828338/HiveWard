...
- Model and harness behavior limits should be expressed through prompts, role contracts, output schemas, examples, and structured context before considering platform code.
...
- Do not build broad platform fallback, normalization, fairness, or correction layers just to make weak model or harness output look better. Bad AI output should be rejected, re-prompted, or surfaced, not silently repaired by the platform.
- Do not add compatibility logic merely to guess model intent or patch over ordinary formatting mistakes. Treat these as prompt/output-contract problems unless they violate core product semantics or safety.
...
- Prefer clear contracts over hidden heuristics. System-level behavior must be explicit in configuration, context, or prompts; do not infer important semantics from names, labels, slot positions, or incidental graph shape.
