...
- Hiveward owns blueprint definitions, canvas state, node configuration, run views, approval state, and local display state.
- OpenClaw owns agents, models, tools, channels, sessions, transcripts, runtime execution, usage facts, and delivery integrations.
- Runtime-specific Gateway/RPC details belong in `packages/adapter`; do not leak protocol mechanics into React components, shared blueprint contracts, or the API worker.
...
- Do not delete user-created blueprints, runs, or catalog snapshots unless explicitly asked.
...
- For chained blueprints, downstream agent nodes must receive upstream outputs through structured input, not just status strings.
...
- Build the usable blueprint command surface, not a marketing shell.
- Keep controls direct: blueprint selector, language toggle, catalog refresh, save, run, inspector, and run results.
...
- Blueprint semantics changes: `npm test`
