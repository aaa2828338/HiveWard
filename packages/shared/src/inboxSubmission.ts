...
export const hivewardInboxSubmissionContract = [
  ...
  "- config.tools: string[]",
  "- runtimeId belongs on the node, not inside config.",
  "- Optional config.modelId selects a configured model for that node's runtime.",
  "- For OpenClaw agent nodes, optional config.openclawAgentId selects the configured OpenClaw Agent.",
  "- Non-OpenClaw nodes must not carry OpenClaw-only openclawAgentId or send configuration.",
  "- Hermes nodes may use config.profileId when a specific Hermes profile is required.",
  "",
  ...
  "- Manager modes are field pairs: sequential is lifecycleMode \"none\" plus dispatchMode \"sequential\"; self_dispatch is lifecycleMode \"none\" plus dispatchMode \"self_dispatch\"; self_iteration is lifecycleMode \"self_iteration\" plus dispatchMode \"self_dispatch\".",
