...
export function buildPromptEnvelope(input: StartAgentTaskInput): string {
  ...
  const schemaText = input.outputSchema
    ? [
        ...
        "If the schema contains humanReportMd, humanReportMd is your free-form human answer: keep the natural report, reasoning, recommendations, caveats, and useful deliverable references there.",
        "Do not force a fixed report template. Use headings, bullets, or plain paragraphs only when they help the current task; for streaming progress, prefer concise natural updates.",
        "If there is a concrete deliverable, mention a real file path, browser URL, or exact artifacts[] reference naturally in humanReportMd and declare it in top-level artifacts[]. If there is no deliverable yet, do not invent a delivery section.",
